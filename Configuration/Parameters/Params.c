...
#include "inverter_card_configuration.h"
...
void PARAMS_InitManual()
{
    ...
    params.motor.P = POLE; // [#]
    params.motor.lq = LQ; // [H]
    params.motor.ld = LD; // [H]
    params.motor.lam = I_AM; // [Wb]
    params.motor.r = R; // [Ohm]
    params.motor.T_max = T_MAX; // [Nm]
    params.motor.i_peak = I_PEAK; // [A]
    params.motor.i_cont = I_CONST; // [A]
    params.motor.id_max = ID_MAX; // [A]
    params.motor.v_nom = LINE_TO_PHASE(VOLTAGE); // [Vpk-ln]
    params.motor.w_nom.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(N_SPEED)), params.motor.P); // [Ra/sec-elec]
    params.motor.w_max.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(MAX_SPEED)), params.motor.P); // [Ra/sec-elec]
#if defined(CTRL_METHOD_SFO)
    params.motor.mtpv_margin = MTPV_MARGIN; // [%]
#endif
    params.motor.i2t.therm_tau = THERM_TAU; // [sec]
    params.motor.i2t.on_level  = ON_LEVEL; // [%]
    params.motor.i2t.off_level = OFF_LEVEL; // [%]
...
#endif
    params.sys.analog.shunt.hyb_mod.adc_t_min = 0.0E-6f; // [sec]
    ...
    params.sys.rate_lim.w_cmd.elec = MECH_TO_ELEC(HZ_TO_RADSEC(RPM_TO_HZ(N_SPEED / 4.0f)), params.motor.P); // [(Ra/sec-elec)/sec]
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    params.sys.rate_lim.i_cmd = I_PEAK * 10.0f; // [A/sec]
#elif defined(CTRL_METHOD_SFO)
    params.sys.rate_lim.T_cmd = T_MAX * 10.0f; // [Nm/sec]
#endif
    // Fault Parameters:
    ...
    params.sys.cmd.virt.switch_time = (1.0f - 0.3f) * 4.0f + 2.0f; // [sec]
#endif
    params.sys.cmd.w_max.mech = HZ_TO_RADSEC(RPM_TO_HZ(N_SPEED)); //[Ra/sec-elec]
#if defined(CTRL_METHOD_RFO) || defined(CTRL_METHOD_TBC)
    params.sys.cmd.i_max = ID_MAX * 2.0f * 0.5f; // [A]
#elif defined(CTRL_METHOD_SFO)
    params.sys.cmd.T_max = T_MAX * 0.5f; // [Nm]
#endif
    ...
    params.mech.inertia = MECH_INERTIA; // [kg.m^2]=[(N.m)/(Ra/sec-mech).sec], =1/2*m*r^2
    params.mech.viscous = MECH_VISCOUS; // [kg.m^2/sec]=[(N.m)/(Ra/sec-mech)]
    params.mech.friction = MECH_FRICTION; // [kg.m^2/sec^2]=[N.m]
    ...
    params.ctrl.volt.v_min = VOLT_VMIN; // [Vpk]
    params.ctrl.volt.v_to_f_ratio = VOLT_VF_RATIO; // [Vpk/(Ra/sec-elec)]
    ...
    params.ctrl.flux_weaken.bw = HZ_TO_RADSEC(15.0f); // [Ra/sec], at least 0.5 dec below current loop's bandwidth
#endif
    ...
    params.ctrl.high_freq_inj.w_h = HZ_TO_RADSEC(params.sys.samp.fs0 * 0.10f); // [Ra/sec], at least 1 decade below switching frequency
    params.ctrl.high_freq_inj.w_sep = HZ_TO_RADSEC(params.sys.samp.fs0 * 0.01f); // [Ra/sec], at least 1 decade below switching frequency
    ...
    params.ctrl.high_freq_inj.i_qd_r_peak.d = ID_MAX; // [A], excitation current peak (d-axis), limiting factor (larger than q-axis)
